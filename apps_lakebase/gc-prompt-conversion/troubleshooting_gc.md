...
- [ ] `app.ts` (pre-Lakebase) uses `import { createApp, server } from "@databricks/appkit"` + `await createApp({ plugins: [server(), ...], async onPluginsReady(appkit) { await registerRoutes(appkit) } })` when **`server/server.ts`** exists — `await` is required, NOT `export default createApp`, NOT `import { server } from "@databricks/appkit/server"` (that subpath crashes). **`server/server.ts`:** routes only inside **`appkit.server.extend((app) => { ... })`** — never **`appkit.server.express`**. Once Lakebase is wired: **`await createApp({ plugins: [lakebase(), server()], async onPluginsReady(appkit) { ... } })`** — **`await` on `createApp` is required** (same reason as pre-Lakebase); no `.then()`, no `server({ autoStart: false })`
...
| Assistant / IDE calls **`appkit_get_app_status`**, **`appkit_list_apps`**, or other **`appkit_*`** MCP tools | Routed to `@appkit-apps` / `@mcp-appkit-tooling` or default tool use | **Stop** — do not use MCP for this workshop. Follow **`one-ui-design-local.md`** refs (**`GENIE-CODE-OVERRIDES`**, **`workshop-variables`**, **`01-appkit-scaffold`**, **`03-appkit-deploy`** only). Status: **`validate_and_deploy`** return value; extra detail: **`w.apps.get(APP_NAME)`** (SDK). |
...
| `ConfigurationError` / app crash — **`lakebase()`** in **`app.ts`** with **`LAKEBASE_ENDPOINT`** / **`valueFrom: postgres`** but **`resources: []`** (no postgres binding) | **Early Lakebase:** Genie applied **Section 6 “With Lakebase”** during **`one-ui-design-local.md`** before **`setup_lakebase_gc.md`** | Revert to **Section 6 “Without Lakebase”** (`plugins: [server()]` only). Remove **`LAKEBASE_*`**, **`DB_SCHEMA`** (Lakebase), and **`valueFrom: postgres`** from **`app.yaml`** until **`setup_lakebase_gc.md`**. Redeploy. Then run **`setup_lakebase_gc.md`** → **`wire_ui_to_lakebase_gc.md`** before **`lakebase()`** again. |
...
| `TypeError: Cannot read properties of undefined (reading 'get')` in `registerRoutes` / `server.ts` | Code uses **`appkit.server.express`** (or assigns `app` from a non-existent AppKit field) — **AppKit does not expose a raw Express instance that way** | In **`registerRoutes(appkit)`**, register every route inside **`appkit.server.extend((app) => { app.get(...); ... })`**. Never call **`app.get`** on **`appkit.server.express`**. See **`GENIE-CODE-OVERRIDES.md` Section 6** (`server/server.ts` — mock stubs) and **`05-appkit-lakebase-wiring`** (`server.extend` gotchas). |
...
- [ ] Default branch database path discovered via `w.postgres.list_databases` (full `projects/.../databases/...` path for app binding — **not** the same string as `DB_SCHEMA`, which is the **Postgres schema** for DDL in `wire_ui_to_lakebase_gc.md`)
...
| `NotFound` / `RESOURCE_DOES_NOT_EXIST` on `get_database_instance(name=APP_NAME)` | First-time setup — instance not created yet | **Expected.** Catch `NotFound` and call `create_database_instance_and_wait(DatabaseInstance(name=APP_NAME))` — see **`setup_lakebase_gc.md` Step 2a** (do not treat as wrong `APP_NAME` unless the app itself is missing). |
| `TypeError: create_database_instance() got an unexpected keyword argument 'name'` | Old snippet: `create_database_instance(name=APP_NAME)` | Use **`DatabaseInstance(name=APP_NAME)`** and **`w.database.create_database_instance_and_wait(...)`** — see **`setup_lakebase_gc.md` Step 2a**. |
| Genie “stuck” on Lakebase / no SDK cells to paste | Prompt pointed at **`04-appkit-plugin-add`** “Steps 3a–3d”, which are **CLI/bundle**, not Genie SDK | Follow **`setup_lakebase_gc.md` Step 2** embedded Python only; upgrade **`databricks-sdk`** if methods are missing. |
| `Identity not found` when creating SP / instance role | Wrong id for the **API you called** (permissions grants often want the SP **UUID**; **`DatabaseInstanceRole.name`** may need numeric **or** UUID per workspace) | Follow **`setup_lakebase_gc.md` Step 2c** exactly: try **`name=str(app.service_principal_id)`** first, then retry **`name=app.service_principal_client_id`**. Do not substitute IDs across unrelated APIs without checking that API’s docs. |
...
| `InvalidParameterValue: Field 'spec.role' cannot be empty` on **`w.postgres.create_database`** | **`Database(spec=DatabaseDatabaseSpec(postgres_database=...))`** without a valid **owner / role** in **`spec`** — control plane requires **`spec.role`** for new databases | **Do not** improvise **`create_database`** here. Use **`setup_lakebase_gc.md` Step 2b** only: **`w.postgres.list_databases(parent=branch_path)`** and bind the **existing** default DB path. |
| `InvalidParameterValue: Field 'spec.role' cannot be empty` (or empty **`postgres_role`**) on **`create_role`** | **`w.postgres.create_role`** with incomplete **`Role.spec`** — server requires **`spec.postgres_role`** (SERVICE_PRINCIPAL identity, etc.) | For this workshop use **`w.database.create_database_instance_role`** only (see **`setup_lakebase_gc.md` Step 2c**). If you must use **`create_role`**, supply a full **`Role(spec=RoleRoleSpec(..., postgres_role=..., membership_roles=[...]))`** with **`parent`** = **branch** path only — never **`.../endpoints/primary`**. |
| `NotFound` on **`POST .../endpoints/primary/roles`** | **`w.postgres.create_role(parent=..., ...)`** used **`parent`** containing **`/endpoints/primary`** — that path is not a valid roles parent | Valid **`parent`** for Postgres roles is the **branch** only, e.g. **`projects/{APP_NAME}/branches/production`**. Prefer **`w.database.create_database_instance_role`** for SP superuser (Step 2c); do not use **`create_role`** with an endpoint parent. |
| `AttributeError: 'str' object has no attribute 'value'` during **`w.apps.update`** / **`resources.append`** | **`AppResourcePostgres.as_dict()`** expects **`permission`** to be an enum and calls **`.value`**; passing **`permission="CAN_CONNECT_AND_CREATE"`** as a raw **`str`** to **`AppResourcePostgres`** triggers this on serialize | Prefer **`setup_lakebase_gc.md` Step 2d** — dict merge + **`App.from_dict(d)`** with **`permission`** as string in the **dict**. If using SDK objects, use **`AppResourcePostgresPostgresPermission.CAN_CONNECT_AND_CREATE`** (enum), not a bare string. |
| Wrong DB after reprovision / binding points at old `db-xxxxx` id | Hardcoded **`DB_PATH`** from an earlier run while the platform created a new default database | Always take **`db_path`** from **`w.postgres.list_databases(parent=branch_path)`** in the same session (Step 2b); never paste a stale `projects/.../databases/db-...` path. |

### Setup Lakebase — extended notes (`setup_lakebase_gc.md`)

**Why `list_databases` only in Step 2b:** Do **not** call **`w.postgres.create_database`** in that step. A default database already exists with the instance. **`create_database`** needs a full **`DatabaseDatabaseSpec`** including an **owner** in **`spec`**; omitting it yields **`InvalidParameterValue: Field 'spec.role' cannot be empty`**. Use **`w.postgres.list_databases(parent=branch_path)`** and bind **`dbs[0].name`**.

**Escape hatch — if `create_database_instance_role` fails or is blocked:** Prefer SDK upgrade or workspace policy. If you must use Postgres roles APIs: **`w.postgres.create_role`** with **`parent=branch_path`** only (`projects/{APP_NAME}/branches/production` — **never** **`.../endpoints/...`**). Use a **complete** [`Role` + `RoleRoleSpec`](https://databricks-sdk-py.readthedocs.io/en/latest/dbdataclasses/postgres.html): **`identity_type=SERVICE_PRINCIPAL`**, **`postgres_role=<app.service_principal_client_id>`** (UUID), **`auth_method=RoleAuthMethod.LAKEBASE_OAUTH_V1`**, **`membership_roles=[RoleMembershipRole.DATABRICKS_SUPERUSER]`**. Set or omit **`role_id`** per current SDK docs.

**`Identity not found`:** Step 2c retries **`DatabaseInstanceRole.name`** as numeric string then UUID. If both fail, the error may be from a **different** API (e.g. UC grants) that expects a specific id shape — check that API’s docs.
...
| **`404`** or empty JSON on **`/api/health/lakebase`** | Genie workshop registers health only at **`GET /api/health`** (`wire_ui_to_lakebase_gc.md`); CLI samples may use a different path | Implement **`GET /api/health`** with wrapped `{ data, source }` shape (`data` array with `status: connected`, `source` live or mock) — do not rely on **`/api/health/lakebase`** unless you add that route explicitly. |
...
| `TypeError: Cannot read properties of undefined (reading 'get')` in `registerRoutes` | Same class as Step 1 — **`appkit.server.express`** or similar undefined handle | Use **`appkit.server.extend((app) => { ... })`** inside `registerRoutes(appkit)` — see **`GENIE-CODE-OVERRIDES.md` Section 6** and Step 1 error table row. |
...
| No **Live / Mock** label in the UI (pages work) | **`ConnectionStatus`** not added per **`05-appkit-lakebase-wiring`** Step 3b — Genie skipped frontend indicator | Add **`ConnectionStatus`** from **`references/frontend-patterns.md`** to **every** page using **`useLakebaseData`**; pass **`source`**. See **`wire_ui_to_lakebase_gc.md`** checklist. |
| **Bookings** journey missing in nav | **`one-ui-design-local.md`** nav shell omitted PRD route; wire step did not add link | Add nav + page wired to **`/api/bookings`** / **`/api/bookings/:id`** as in PRD; see **`wire_ui_to_lakebase_gc.md`** route table. |
