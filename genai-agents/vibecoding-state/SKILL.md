...
All twelve operations take a typed parameter bag. The prompt invokes the operation by name (e.g. `vibecoding-state.enter`) and passes only the dynamic values. Everything else (file paths, log schema, gate rules, pathway matrix, resolved spec) is owned by this skill.
...
1. **CLI-version gate.** If `min_cli_version` is set, halt early when `bootstrap_preflight.databricks_cli_version < min_cli_version` AND no candidate kind in `candidates` is `import_path` or `function_signature` (the two CLI-independent kinds). The remediation hint MUST cite the upgrade path captured by `bootstrap` step 3.
