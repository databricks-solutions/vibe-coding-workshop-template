...
**✅ Recommended:** Use the module-level cache pattern with `toPandas()` to avoid DLT static analysis warnings.
...
## Avoiding the `DataFrame.collect()` Warning in DLT
...
**See:** The `dq_rules_loader.py` example above uses this pattern with `toPandas()` and module-level caching.
