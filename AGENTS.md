...
> **This file is the universal entry point for any AI coding assistant** — Cursor, Claude Code, Windsurf, Copilot, Codex, Genie Code, or any agent that reads `AGENTS.md`.
...
├── pre-req-mcp-setup.md                <-- MCP AppKit admin setup (`mcp-appkit-skill` + OAuth scope)
...
| Keywords | Read This |
|----------|-----------|
| "scaffold", "create app", "new app", "AppKit", "init app" | `apps_lakebase/skills/01-appkit-scaffold/SKILL.md` |
| "build app", "implement UI", "frontend", "backend", "PRD" | `apps_lakebase/skills/02-appkit-build/SKILL.md` |
| "deploy app", "app.yaml", "Databricks Apps", "validate app" | `apps_lakebase/skills/03-appkit-deploy/SKILL.md` |
| "Lakebase", "PostgreSQL", "database tables", "persistence" | `apps_lakebase/skills/00-appkit-navigator/SKILL.md` |
| "wire lakebase", "DDL", "CRUD API", "useLakebaseData" | `apps_lakebase/skills/05-appkit-lakebase-wiring/SKILL.md` |
| "add plugin", "analytics plugin", "genie plugin", "setup lakebase" | `apps_lakebase/skills/04-appkit-plugin-add/SKILL.md` |
| "workshop guide", "full lifecycle", "phases", "Instructions" | `apps_lakebase/Instructions.md` (overview) + `apps_lakebase/prompts/README.md` (Genie order) |

#### Local IDE + CLI (Cursor, VS Code, Terminal)
...
| "Lakebase", "PostgreSQL", "database tables", "persistence" | `apps_lakebase/skills/00-appkit-navigator/SKILL.md` |
| "wire lakebase", "DDL", "CRUD API", "useLakebaseData" | `apps_lakebase/skills/05-appkit-lakebase-wiring/SKILL.md` |
| "add plugin", "analytics plugin", "genie plugin" | `apps_lakebase/skills/04-appkit-plugin-add/SKILL.md` |
| "workshop guide", "full lifecycle", "phases", "Instructions" | `apps_lakebase/Instructions.md` (overview) + `apps_lakebase/prompts/README.md` (Genie order) |
...
6. **For AppKit tasks**, read `apps_lakebase/skills/`; in Genie Code, apply `apps_lakebase/gc-prompt-conversion/GENIE-CODE-OVERRIDES.md`. For local CLI flows, follow the same skills with your local toolchain.
...
| IDE / Agent | How It Discovers This File | File Reference Syntax | Skill Variant |
|-------------|---------------------------|----------------------|--------------|
| **Genie Code** | Reads `AGENTS.md` in workspace | SDK file paths | `apps_lakebase/skills/` |
| **Cursor** | Auto-loads `AGENTS.md` | `@path/to/file` | `apps_lakebase/skills/` |
| **Claude Code** | Reads `AGENTS.md` (or `CLAUDE.md`) at repo root | Reference files by path | `apps_lakebase/skills/` |
| **VS Code / Copilot** | Reads `AGENTS.md` or `.github/copilot-instructions.md` | `#file:path/to/file` | `apps_lakebase/skills/` |
| **Windsurf** | Reads `AGENTS.md` or `.windsurfrules` at repo root | `@path/to/file` | `apps_lakebase/skills/` |
| **Codex** | Reads `AGENTS.md` at repo root | Reference files by path | `apps_lakebase/skills/` |
| **Other** | Point the agent to this file manually | Paste file contents or path | `apps_lakebase/skills/` |
...
### Databricks AppKit App (CLI or Genie Code)
...
```
(Genie Code: also read `apps_lakebase/gc-prompt-conversion/GENIE-CODE-OVERRIDES.md` and `apps_lakebase/prompts/README.md`.)
